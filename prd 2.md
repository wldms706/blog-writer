...
#### 6-4. 주제 기반 키워드 추천 ✅ (NEW)
- 선택한 주제(topic)에 맞는 키워드 우선 추천
- 예: 아이라인 주제 → "천안 아이라인", "서북구 아이라인문신" 추천
- 업종+주제별 키워드 매핑 데이터 구축
- 주제와 무관한 키워드(예: 눈썹문신) 추천 방지
...
#### 단어 반복 제한 (반영구 업종) ✅ (NEW)
- "눈썹", "문신", "반영구" 각 단어 10회 이하 사용
- 과도한 반복 시 네이버 저품질 처리 위험
- 동의어 대체 권장:
  - "눈썹" → "브로우", "눈매", "이마 라인"
  - "문신" → "시술", "아트", "메이크업"
...
| 자기소개 | 철학/신념 |
|---------|----------|
| 경력이 어떻게 되시나요? | 가장 중요한 가치는? |
| 전문 분야는? | 다른 샵과 다른 점? |
| 시작한 계기? | 왜 그런 철학을? |
| 고객 대하는 마음? | 전하고 싶은 메시지? |
...
### 11. SEO 글쓰기 스타일 가이드 ✅ (NEW)

실제 SEO 노출 성공 사례 5개를 분석하여 도출한 글쓰기 규칙:

#### 성공 패턴 ✅
- **짧은 문장**: 한 문장 20~30자 내외, 모바일 읽기 편하게
- **관찰자 시점**: "관찰해보면...", "살펴보면...", "많은 분들이 ~하지만..."
- **관점 제시**: 정보 나열 대신 "왜 중요한지", "어떤 기준으로 봐야 하는지"
- **질문으로 마무리**: "예약하세요" 대신 "스스로에게 이 질문을 던져보세요"

#### 절대 금지 패턴 ❌
- **가상 인물 스토리**: "30대 직장인 김민지 씨는..." 금지
- **소설식 전개**: 꾸며낸 시나리오, 가상 경험담 금지
- **AI 문체**: "~하는 것이 중요합니다. 또한 ~도 고려해야 합니다" 금지
- **결론 없는 글**: "아직 시술을 받은 것은 아니지만..." 금지

#### 글 구조 유형 (5가지) - 랜덤 선택
| 유형 | 설명 |
|------|------|
| **경험-사례형** ⭐ | 실제 상담 사례로 시작 → 검색자 의도 → 변수 맵 → 판단 체크리스트 → 선 긋기 → 다음 행동 제시 |
| 해결책 중심형 | 문제 → 해결 방향 → 기준 제시 |
| 상황별 안내형 | 상황마다 다른 접근 필요 설명 |
| 통찰-관점형 | 전문가 시각으로 새로운 관점 제시 |
| 체크포인트형 | 확인해야 할 중요 사항 설명 |

#### 경험-사례형 글 구조 (6단계) ✅ (NEW)
```
1. 상황 훅 (경험/사례) - 실제 상담 사례나 현장 에피소드로 시작
2. 검색자 의도 확정 - 표면 질문 뒤 진짜 고민 짚어주기
3. 변수 맵 - 케이스가 갈리는 이유 3~5개 (연관 키워드 분산)
4. 판단 체크리스트 - 독자가 스스로 판단할 수 있는 기준 제시
5. 선 긋기 + 예외 인정 - 무조건적 권유 X, 상황별 안내
6. 다음 행동 제시 - 질문으로 끝내되 자연스러운 CTA 연결
```

#### 언어 규칙 ✅
- 반드시 한국어로만 작성
- 영어, 러시아어, 중국어 등 다른 언어 혼입 금지
...
| 주제 | 설명 |
|------|------|
| 헤어스트로크 | 최상위 기법, 자연스럽고 오래 유지 |
| 머신페더링 | 최상위 기법, 덜 아프고 자연스러운 결 |
| 엠보 | 수작업 결 표현, 클래식 기법 |
| 머신쉐딩 | 그라데이션 음영 표현 |
| 수지 | 수지 기법, 자연스러운 눈썹 |
| 콤보/복합 | 엠보+쉐딩 등 복합 기법 |
...
| 헤어라인 | 헤어라인 교정 시술 |
| 입술 | 입술 반영구 시술 정보 |
| **SMP(두피문신)** | 두피 마이크로피그먼테이션, 탈모 커버 ⭐NEW |
| 리터치/수정 | 리터치 주기, 수정 관리 |
| 왁싱 | 🪶 | X |
| **속눈썹** | 👁️ | X |
...
- [x] 카카오 로그인
- [x] Google 로그인
- [x] Supabase OAuth Provider 설정
- 이메일/비밀번호 로그인 제거, 소셜 로그인만 지원
...
| 날짜 | 내용 |
|------|------|
| 2026-02-03 | **SMP(두피문신) 주제 추가** - 반영구 업종에 SMP 주제 및 키워드 매핑 추가 |
| 2026-02-03 | **주제 기반 키워드 추천** - 선택한 주제(아이라인 등)에 맞는 키워드 우선 추천 |
| 2026-02-03 | **경험-사례형 글 구조 추가** - 6단계 구조 (상황훅→검색의도→변수맵→판단체크→선긋기→다음행동) |
| 2026-02-03 | **단어 반복 제한** - 반영구 업종 눈썹/문신/반영구 10회 이하 사용 |
| 2026-02-03 | **Next.js 16.1.6 업데이트** - 보안 취약점 수정 |
| 2026-02-01 | **한국어만 사용 규칙** - 영어/러시아어/중국어 등 혼입 방지 |
| 2026-02-01 | **SEO 글쓰기 스타일 가이드** - 실제 성공 사례 5개 분석 기반 규칙 적용 |
| 2026-02-01 | **소설식 글 구조 제거** - 가상 인물/스토리 SEO 노출 실패로 제거, 통찰-관점형으로 대체 |
| 2026-01-31 | **소셜 로그인 구현** - 카카오, Google 로그인 (이메일/비밀번호 제거) |
| 2026-01-30 | **브랜딩 종류 간소화** - 샵 스토리 제거, 자기소개/철학만 유지 |
