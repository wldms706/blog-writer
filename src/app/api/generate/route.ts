...
- 각 소제목 섹션을 3문단 이상 충실하게 쓰세요`;
      ...
      // SEO 글 생성 — 규제 업종 or 전문가형은 기존 프롬프트, 후기형은 체험 프롬프트
      const useExperienceStyle = !isRegulatedBusiness && seoStyle === 'review';
      systemPrompt = isRegulatedBusiness
        ? SEO_REGULATED_PROMPT
        : useExperienceStyle
...
⚠️ 글의 목적과 독자 상태를 반드시 반영하세요:
- 글의 목적이 "${safePurpose}"이므로, 글 전체가 이 목적을 달성하는 방향으로 작성하세요
- 독자는 지금 "${safeReaderState}" 상태입니다. 이 독자의 심리에 맞춰서 도입부와 톤을 조절하세요
- 주제 "${safeTopic}"에 집중하세요. 주제와 무관한 내용은 넣지 마세요
