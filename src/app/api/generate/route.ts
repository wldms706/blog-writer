...
// 소설식 글 프롬프트 (반영구 전용)
const STORY_STYLE_PROMPT = `당신은 공감되는 스토리텔링 블로그 글 작성 전문가입니다.

## 소설식 글 특성
가상의 인물을 주인공으로 한 스토리텔링 형식의 글을 작성합니다.
독자가 "나도 저런 고민 있었는데!" 하고 공감하면서 자연스럽게 정보를 얻어가는 글입니다.

## ✅ 올바른 글쓰기 방향
- 3인칭 가상 인물의 시점으로 이야기 전개
- "민지씨는", "영희는", "한 30대 직장인은" 등 가상 캐릭터 활용
- 독자가 공감할 수 있는 일상적인 상황 묘사
- 이야기 속에 자연스럽게 정보와 인사이트 녹여내기
- 소설처럼 읽히지만, 결국 유용한 정보를 전달하는 글

## ✅ 글 구조
1. 도입: 가상 인물의 일상/고민 묘사 (공감 포인트)
2. 전개: 고민이 깊어지거나 해결책을 찾아가는 과정
3. 발견: 새로운 정보나 인사이트를 알게 되는 장면
4. 심화: 그 정보에 대해 더 자세히 알아보는 내용
5. 마무리: 인물의 결론 + 독자에게 도움이 되길 바라는 메시지

## ⚠️ 규제 업종 필수 준수 (반영구/눈썹문신)
- 자기 홍보 절대 금지: 특정 샵이나 시술자 언급 금지
- 1인칭 금지: "저는", "제가", "저희" 사용 금지
- 결과/후기 언급 금지: "만족했다", "결과가 좋았다" 금지
- 영업 유도 금지: "예약하세요", "방문해보세요" 금지

## ✅ 올바른 예시
- "강남역 근처에서 일하는 수진씨는 매일 아침 눈썹 그리는 시간이 고민이었다." (O)
- "여름이 되자 지영씨의 고민은 더 깊어졌다. 땀에 지워지는 눈썹..." (O)
- "검색창에 '눈썹문신'을 쳐본 건 그때가 처음이었다." (O)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (contentType === 'branding') {
      // 브랜딩 글 생성 - 소설식은 별도 프롬프트, 규제 업종(반영구)은 1인칭 금지, 나머지는 1인칭 허용
      if (brandingType === 'story') {
        systemPrompt = STORY_STYLE_PROMPT;
      } else {
...
- 고객에게 전하고 싶은 메시지: ${message || '미입력'}`;
          ...
          const { character, situation, season, insight } = info;
          detailedInfo = `
## 소설 속 인물 설정 (이 설정을 바탕으로 스토리를 전개하세요)
- 주인공: ${character || '30대 직장인'}
- 상황/고민: ${situation || '매일 눈썹 그리기 힘듦'}
- 시기/계절: ${season || '미정'}
- 전달할 정보/인사이트: ${insight || '눈썹문신에 대한 정보'}`;
        }
      }
...
${brandingType === 'story' ? `
### 소설식 글 구조:
⚠️ 가상 인물의 시선으로 스토리텔링 형식의 글을 작성하세요.

1. 도입: 주인공의 일상과 고민 묘사
   - "${brandingInfo?.story?.character || '주인공'}"의 평범한 하루 시작
   - "${brandingInfo?.story?.situation || '눈썹 관련 고민'}" 상황 묘사
   - 독자가 "나도 그래!" 하고 공감할 수 있게

2. 전개: 고민이 깊어지는 상황
   - ${brandingInfo?.story?.season ? `"${brandingInfo.story.season}" 시기의 특수한 상황` : '계절이나 특별한 시기'}
   - 주변 반응이나 불편함 묘사
   - 해결책을 찾아보기 시작

3. 발견: 정보를 알게 되는 장면
   - 검색, 지인 대화, 우연한 발견 등
   - "${brandingInfo?.story?.insight || '눈썹문신에 대한 정보'}" 내용 자연스럽게 풀어내기
   - "이건 몰랐네" 하는 깨달음 포인트

4. 심화: 더 자세한 정보 탐색
   - 관련 지식 상세 설명 (기법 종류, 선택 기준 등)
   - 주인공의 생각과 판단 과정

5. 마무리: 결론과 독자에게 메시지
   - 주인공의 결론 (결정했든 고민 중이든)
   - 비슷한 고민 있는 분들께 도움 되길 바라는 메시지

## 주의사항:
- 3인칭으로 작성 (주인공 이름 또는 "그녀는", "그는" 사용)
- 소설처럼 생생하게 묘사하되, 정보도 충실히 전달
- 특정 샵이나 시술자 홍보 절대 금지
` : ''}
...
${brandingType === 'story' ? `
## 소설식 글 필수 준수사항
- 3인칭 가상 인물 시점으로 작성 (1인칭 금지)
- 자기 홍보 절대 금지: 특정 샵, 시술자 언급 금지
- 결과/후기 언급 금지: "만족했다", "결과가 좋았다" 금지
- 소설처럼 생생하게 묘사하되, 유용한 정보도 자연스럽게 포함
- 독자가 공감하며 읽을 수 있는 스토리 전개` : (isRegulatedBusiness ? `
