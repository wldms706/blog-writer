...
${storyParts.map(s => `- ${s}`).join('\n')}\n`
    ...
    // 후기형이면 규제 업종에서도 샵 정보 허용 (손님 후기이므로)
    const allowShopInfo = !isRegulatedBusiness || seoStyle === 'review';
...
⚠️⚠️⚠️ 글자수 (절대 규칙):
- [편집가이드: ...] 줄을 전부 삭제한 뒤 순수 본문만 공백 제외 2,000자를 목표로 작성하세요
- 1,500자 미만이면 거부입니다
- 각 소제목 섹션을 3문단 이상 충실하게 쓰세요`;

    } else {
      // SEO 글 생성 분기:
      // - 휴먼톤(human): AI 티 안 나는 진짜 사람 글 (최우선)
      // - 후기형(review): 손님 체험 후기 (규제/비규제 무관)
      // - 전문가형(expert) + 규제: 전문 정보 전달 (기존 반영구 규칙 적용)
      // - 전문가형(expert) + 비규제: 원장 시점 정보 공유
      const useHumanStyle = seoStyle === 'human';
      const useExperienceStyle = seoStyle === 'review';
      systemPrompt = useHumanStyle
        ? `${SEO_HUMAN_PROMPT}${toneOverride}`
        : useExperienceStyle
          ? `${SEO_EXPERIENCE_PROMPT}${toneOverride}`
          : isRegulatedBusiness
            ? SEO_REGULATED_PROMPT
            : `${SEO_REVIEW_STYLE_PROMPT}${toneOverride}`;

      if (useHumanStyle) {
        // 리얼 휴먼 톤: 간결한 user prompt + SEO_HUMAN_PROMPT가 톤 담당
        userPrompt = `다음 조건에 맞는 진짜 사람이 쓴 것처럼 자연스러운 블로그 글을 작성해주세요.
...
⚠️ 시스템 프롬프트의 "리얼 휴먼 톤" 가이드를 반드시 지켜주세요:
- "정말", "확실히", "솔직히" 같은 부사 절대 금지
- 정연한 1-2-3 구조 금지
- "여러분", "~하시기 바랍니다" 절대 금지
- 결론 흐지부지 OK, 질문으로 끝내기 OK
- "..." "ㅋㅋ" "ㅠㅠ" 자연스럽게
- 구체적 디테일 (시간, 숫자, 사람) 살리기
- 갑자기 옆길로 새는 부분 만들기
- 문장 길이 들쭉날쭉하게

${safeCustomPurpose ? `⚠️ 반드시 전달할 내용: ${safeCustomPurpose}\n` : ''}${safeTreatmentInfo ? `⚠️ 반드시 포함할 시술 정보: ${safeTreatmentInfo}\n` : ''}${shopInfoText ? `⚠️ 가게 정보 (자연스럽게 글 안에 녹이세요):\n${shopInfoText}\n${shopPhotoGuideText}\n` : ''}

⚠️ 키워드 "${safeKeyword}" 4~6회 (통째로) + 쪼개서 2~3회 자연스럽게 분산
⚠️ 본문 1500자 이상 (편집가이드 제외)
⚠️ 마크다운 금지, 순수 텍스트만`;
      } else if (isRegulatedBusiness && !useExperienceStyle) {
        // 규제 업종: 기존 전문가 시점 프롬프트 유지
        const diversity = getRandomDiversityKit();

        userPrompt = `다음 조건에 맞는 전문 정보 블로그 글을 작성해주세요.

키워드: ${safeKeyword}
업종: ${safeBusinessCategory}
글 주제: ${safeTopic}

⚠️ 주제 "${safeTopic}"에 집중하세요. 주제와 무관한 내용은 넣지 마세요

⚠️⚠️⚠️ 업종 범위 제한 (절대 규칙):
이 글은 "${safeBusinessCategory}" 업종의 글입니다.
- "${safeBusinessCategory}" 업종에 해당하는 내용만 작성하세요
- 다른 업종의 내용을 절대 섞지 마세요
${safeSelectedTitle ? `제목: ${safeSelectedTitle}
...
${shopPhotoGuideText}` : ''}`;
      ...
      } else if (useExperienceStyle) {
        // 후기형 (규제/비규제 무관): 손님 체험 후기 프롬프트
        const expDiversity = getExperienceDiversityKit();

        userPrompt = `다음 조건에 맞는 체험 후기 스타일 블로그 글을 작성해주세요.
...
이 글은 "${safeBusinessCategory}" 업종의 글입니다.
- "${safeBusinessCategory}" 업종에 해당하는 내용만 작성하세요
- 다른 업종(두피, 헤어, 네일, 왁싱 등)의 내용을 절대 섞지 마세요
- 키워드 "${safeKeyword}"와 업종 "${safeBusinessCategory}"에 100% 맞는 내용만 작성하세요
${safeSelectedTitle ? `제목: ${safeSelectedTitle}
...
- 제목에서 언급한 핵심 포인트가 본문의 중심 주제가 되어야 합니다
- 제목과 무관한 내용으로 본문을 채우면 글 전체 거부됩니다
- 도입부 첫 2~3문장에서 제목의 맥락을 바로 이어가세요` : ''}

## 이번 글의 도입부 (반드시 이 방식으로 시작하세요)
${expDiversity.introStyle}

## 소제목 스타일
${expDiversity.headingStyle}

## 이번 글의 톤 (글 전체에 이 톤을 유지하세요)
${expDiversity.tone}

## 체험 후기 필수 요소
- 화자는 "체험자/방문자" — 1인칭으로 자연스럽게 경험을 서술
- 감정 흐름: 고민 → 검색 → 발견 → 방문 → 시술 → 결과 → 추천
- 시술자(원장/선생님) 대화를 큰따옴표로 직접 인용 최소 3회
- 짧은 문장 (2~3줄) + 문단마다 빈 줄
- 업체 정보(이름, 주소, 지도 위치)를 글 초반~중반에 자연스럽게 배치
- 시술 과정을 3~5단계로 나눠서 감각 묘사와 함께 상세히
- 비포/애프터 변화 묘사
- 애프터케어(관리법, 주의사항) 안내로 실용성 추가
- 마무리는 "정착할 생각", "재방문 의향", "다음엔 뭐할지 기대" 등으로
...
${safeTreatmentInfo ? `
⚠️⚠️ 필수 반영: 이 샵의 특별한 시술/프로그램 정보
아래 내용을 체험자가 직접 경험한 것처럼 자연스럽게 녹여서 포함하세요.
시술자가 설명해준 내용을 대화 인용으로 녹이면 가장 자연스럽습니다:
- "선생님이 ○○ 방식으로 진행해주셨는데, 확실히 다르더라고요"
- "원장님이 '이 기법은 ○○해서 결과가 더 좋다'고 설명해주셨어요"
...
- 업종 단어는 각각 10회 이하
${isRegulatedBusiness ? `
⚠️ 반영구/눈썹문신 업종 손님 후기 주의사항:
- 시술자가 쓰는 글이 아닌, 손님이 직접 방문해서 쓴 후기 톤 유지
- "원장님이 시술해주셨다", "선생님이 해주셨다" 등 손님 시점 표현 사용
- 1인칭 "저는", "제가"는 손님 기준으로만 사용 (샵 주인이 아님)
- 의료 행위 느낌 표현 금지: "치료", "진료", "처방", "환자" 등 대신 "관리", "상담", "고객" 사용
- 과장된 효과 표현 금지: "완벽", "부작용 0%", "영구" 등` : ''}
${shopInfoText ? `
⚠️ 샵 정보 (글 초반~중반에 자연스럽게):
"검색하다가 발견한 곳", "후기 좋은 곳으로 골랐다" 등의 맥락으로 아래 정보를 녹여주세요.
${shopInfoText}
${shopPhotoGuideText}` : ''}`;
      } else {
        // 비규제 업종 + 전문가형: 기존 시술자 관점 프롬프트
        const diversity = getRandomDiversityKit();

        userPrompt = `다음 조건에 맞는 시술자 관점의 블로그 글을 작성해주세요.
...
${safeCustomPurpose ? `⚠️⚠️ 최우선 반영 필수: ${safeCustomPurpose}` : ''}
${safeTreatmentInfo ? `⚠️⚠️ 필수 반영 시술 정보: ${safeTreatmentInfo}` : ''}

⚠️ 업종 키워드 반복 제한: 각각 10회 이하
${shopInfoText ? `⚠️ 샵 정보 (글 마지막에 짧게): ${shopInfoText}${shopPhotoGuideText}` : ''}`;
      }

      // 공통 부분: 제목, 키워드 배치, 글자수
      userPrompt += `

위 조건을 반영하여, ${isRegulatedBusiness ? '독자에게 깊이 있는 전문 정보를 전달하는 글' : useExperienceStyle ? '실제 방문 체험 후기처럼 자연스럽고 생동감 있는 글' : '시술자가 본인 블로그에 쓰는 자연스러운 정보 공유 글'}을 작성해주세요.
⚠️ 글의 첫 줄에 반드시 제목을 쓰세요:
${safeSelectedTitle ? `첫 줄: ${safeSelectedTitle}
(이 제목을 반드시 글의 맨 첫 줄에 그대로 출력하세요)
그 다음 한 줄 비우고 본문을 시작하세요.
- 제목이 던지는 질문/주장을 본문에서 반드시 답하세요` : '직접 제목을 만들어서 첫 줄에 쓰고, 한 줄 비운 후 본문을 시작하세요.'}
...
- 배치 규칙 (반드시 골고루 분산):
  ① 도입부 (첫 2~3문장 내) — 반드시 1회
  ② 본문 중간 — 반드시 2~3회 분산
  ③ 마무리 (마지막 2~3문장 내) — 반드시 1회
- 한 문단에 2회 이상 몰아넣기 절대 금지
` : `
⚠️⚠️⚠️ 키워드 "${safeKeyword}" 배치 (네이버 노출 핵심 — 이 규칙 어기면 글 전체 거부):
- 제목에 반드시 "${safeKeyword}" 1회 포함 (필수)
...
⚠️⚠️⚠️ 글자수 (절대 규칙):
- [편집가이드: ...] 줄을 전부 삭제한 뒤 순수 본문만 공백 제외 ${isRegulatedBusiness ? '2,000' : '2,500'}자를 목표로 작성하세요
- 1,500자 미만이면 거부입니다
- 각 섹션을 3문단 이상 충실하게 쓰세요`;
    }
...
1. [편집가이드] 줄을 제외한 순수 본문을 공백 제외 최소 1,800자 이상 작성하세요. 1,500자 미만이면 실패입니다.
2. 글을 짧게 끝내지 마세요. 각 섹션을 3문단 이상 충실하게 작성하세요.
3. 마크다운 금지. 순수 텍스트만 출력하세요.
4. 현재 연도: ${currentYear}년`,
        messages: [
