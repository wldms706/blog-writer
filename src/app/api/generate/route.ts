...
- 예: 피부관리 업종이면 피부관리만, 속눈썹 업종이면 속눈썹만, 헤어 업종이면 헤어만
${safeSelectedTitle ? `제목: ${safeSelectedTitle}
...
- 배치 규칙 (반드시 골고루 분산):
  ...
  ② 본문 중간 소제목 1~2번 구간 — 반드시 1~2회
  ③ 본문 중간 소제목 3~4번 구간 — 반드시 1~2회
...
- 배치 규칙 (반드시 골고루 분산):
  ① 도입부 (첫 2~3문장 내) — 반드시 1회
  ② 본문 중간 소제목 1~2번 구간 — 반드시 1회
  ③ 본문 중간 소제목 3~4번 구간 — 반드시 1회
  ④ 마무리 (마지막 2~3문장 내) — 반드시 1회
- 한 문단에 2회 이상 몰아넣기 절대 금지
- 마지막 문단에만 키워드 몰아넣으면 글 전체 거부
- "${safeKeyword}"를 정확히 그대로 써야 함 (띄어쓰기, 순서 변경 금지)
`}
