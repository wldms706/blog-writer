...
- 각 소제목 섹션을 3문단 이상 충실하게 쓰세요`;
      ...
      // SEO 글 생성
      systemPrompt = isRegulatedBusiness ? SEO_REGULATED_PROMPT : `${SEO_EXPERIENCE_PROMPT}${toneOverride}`;
...
${shopPhotoGuideText}` : ''}`;
        ...
        // 비규제 업종: 체험 후기형 프롬프트 (네이버 상위노출 최적화)
...
위 조건을 반영하여, ${isRegulatedBusiness ? '독자에게 깊이 있는 전문 정보를 전달하는 글' : '실제 방문 체험 후기처럼 자연스럽고 생동감 있는 글'}을 작성해주세요.
