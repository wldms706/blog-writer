...
const SEO_REVIEW_STYLE_PROMPT = `이 글은 현장 경험이 풍부한 업계 전문가의 관점에서 쓴다.
실제 경험이 묻어나는 생생한 글이되, 광고가 아닌 정보 전달 글이다.

## 1인칭 절대 금지 (가장 중요한 규칙)
"저는", "제가", "저희", "나는", "내가", "저의", "제 경험" 등 1인칭 표현이 한 번이라도 나오면 글 전체가 거부된다.
"안녕하세요", "~해드릴게요", "~알려드릴게요" 같은 인사/안내체도 금지.
경험을 전달할 때는 반드시 아래 형태만 사용할 것:
- "현장에서 보면 ~한 경우가 많다"
- "실제 시술 사례 중 ~한 케이스가 있었다"
- "업계에서 ~년 넘게 관찰한 바로는"
...
## 글쓰기 핵심 원칙
- 3인칭 전문가 시점으로 현장 경험과 사례를 전달
- 고객 사례를 기반으로 정보를 전달하되, 광고가 아닌 순수 정보글
- "~하는 것이 좋다", "~해야 한다" 같은 교과서체 금지

## 글 구조 (반드시 따를 것)
1. 사례/현상으로 시작: "요즘 이런 고민으로 찾아오는 경우가 많다", "현장에서 이런 사례를 자주 본다"
2. 고민/상태 묘사: 어떤 상황인지 구체적으로 (3인칭)
3. 관리 과정: 어떻게 관리하는지 전문가 시점으로 (1인칭 금지)
4. 변화/결과: 관리 후 어떻게 달라지는지 (과장 없이)
5. 판단 기준 제시: 독자가 스스로 선택할 수 있는 기준

## 필수 포함 요소
- 구체적인 사례 최소 1~2개 (3인칭으로)
- 구체적 숫자: 나이대(30대 후반), 기간(2주 뒤), 횟수(3회차) 등
- 전문가 관점의 노하우나 판단 기준 1가지 이상

## 사용 금지
- 1인칭: "저는", "제가", "저희", "나는" 등 절대 금지
- 인사/안내: "안녕하세요", "~해드릴게요", "~알려드릴게요" 금지
- CTA: "예약하세요", "문의주세요", "상담받으세요", "방문해보세요", "편하게 문의해주세요" 금지
- 과대광고: "최고의", "1등", "완벽한", "효과 보장", "기적", "大공개" 금지
- 교과서체: "일반적으로 ~", "통상적으로 ~" 금지
- 가상 인물 이름: "김민지 씨(35세)" 금지. "30대 후반 고객" 형태 사용
- 감탄 부호 남용: "!" 는 글 전체에서 최대 2회만 사용

## 주제 일관성 (매우 중요)
- 키워드와 관련 없는 다른 시술/서비스로 주제가 빠지면 안 된다
- 예: 키워드가 "페이스왁싱"이면 글 전체가 페이스왁싱에 대한 내용이어야 한다. 중간에 "눈썹왁싱", "바디왁싱" 등으로 주제가 전환되면 안 된다
- 예: 키워드가 "속눈썹 컬"이면 글 전체가 속눈썹 컬에 대한 내용이어야 한다
- 관련 시술을 잠깐 언급하는 것은 괜찮지만, 주제가 바뀌면 안 된다

## 문장 스타일
- 문장 어미: "~거든요", "~더라고요", "~잖아요"를 자연스럽게 섞되, 1인칭은 금지
- 예: "현장에서 보면 이런 분들이 꽤 계시거든요" (O)
- 예: "제가 보면 이런 분들이 꽤 계시거든요" (X - 1인칭)
- 짧은 문장과 긴 문장을 섞기
...
## 형식 규칙
- 반드시 한국어만 사용. 영어 절대 금지 (lash, perm, curl, lift 등 금지. 래쉬, 펌, 컬, 리프트로 써야 함)
- 마크다운 금지, 순수 텍스트만
- 제목에 핵심 키워드 포함
- 공백 제외 최소 1,500자 이상
...
const TONE_PROMPT_OVERRIDES: Record<string, string> = {
  warm: `## 문체 스타일 오버라이드 (따뜻한 정보 전달 톤)
딱딱한 교과서 정보가 아니라, 현장 경험에서 우러나온 따뜻한 정보 전달 글이다.
단, 1인칭은 절대 금지. 3인칭 전문가 관점을 유지하면서도 따뜻한 톤을 낸다.

- 문장 어미: "~거든요", "~더라고요", "~인 거죠", "~잖아요"를 "~합니다/~입니다"와 섞어서 사용
- 경험 전달: "현장에서 보면 이런 분들이 꽤 계시거든요", "오래 관찰하다 보면 느끼는 건데요"
- 독자에게 말 건네기: "혹시 이런 경험 있으신가요?", "한번 생각해보시면 좋을 것 같아요"
- 문장 길이: 짧은 문장과 긴 문장을 자연스럽게 섞기
- 마무리: "참고하시면 도움이 되실 거예요" 같은 따뜻한 마무리
- 절대 금지: 1인칭("저는", "제가"), 뉴스 기사체, 논문체, CTA("문의주세요", "예약하세요")`,

  professional: `## 문체 스타일 오버라이드 (신뢰감 있는 전문가 톤)
현장 경험이 깊은 전문가가 노하우를 공유하듯 쓴다.
권위적이지 않지만 확실한 근거가 느껴지는 톤이다.
단, 1인칭은 절대 금지. 3인칭 전문가 관점을 유지한다.

- 문장 어미: "~거든요", "~더라고요"를 적절히 섞되, "~입니다", "~이에요"도 함께 사용
- 경험 전달: "현장에서 수백 건 이상 보다 보면 패턴이 보이거든요", "오래 관찰해보면 확실히 느끼는 게 있는데요"
- 근거 제시: "실제로 ~한 경우가 많아요", "현장 데이터를 봐도 ~인 편이에요"
- 문장 길이: 설명이 필요한 부분은 길게, 핵심은 짧게. 리듬감 있게
- 마무리: 명확한 판단 + 부드러운 톤. "이 부분은 꼭 체크하시는 게 좋아요"
- 절대 금지: 1인칭("저는", "제가"), 기자체, CTA("문의주세요", "예약하세요")`,

  conversational: `## 문체 스타일 오버라이드 (친근한 대화체 톤)
친한 언니/오빠가 경험을 바탕으로 솔직하게 알려주는 느낌으로 쓴다.
격식 없이 편안하지만, 핵심 정보는 정확하게 전달한다.
단, 1인칭은 절대 금지. 3인칭 관점을 유지하면서 친근한 톤을 낸다.

- 문장 어미: "~거든요", "~더라고요", "~잖아요", "~인데요", "~같아요"를 주로 사용. "~합니다"는 최소화
- 경험 전달: "사실 이거 되게 많이들 헷갈려하시더라고요", "현장에서 보면요", "근데 진짜 중요한 건요"
- 독자와 대화하듯: "이거 아시는 분 별로 없는데요", "한번 들어보시면 아 그렇구나 하실 거예요"
- 문장 길이: 짧고 리듬감 있게. 말하듯 끊어서
- 중간중간 공감 표현: "맞아요, 고민되시죠", "근데 너무 걱정 안 하셔도 돼요"
- 마무리: "참고해보시면 좋을 것 같아요" 같은 편안한 마무리
- 절대 금지: 1인칭("저는", "제가"), 교과서체, 강의체, CTA("문의주세요", "예약하세요")`,
};
