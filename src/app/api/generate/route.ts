...
// 외국어(영어, 러시아어 등) 단어를 제거하는 필터
function filterForeignWords(text: string): string {
  // 러시아어(키릴 문자) 단어 제거
  let filtered = text.replace(/[а-яА-ЯёЁ]+/g, '');
  // 영어 단어 제거 (2글자 이상 연속 영문자). 단, SEO/CTA 같은 업계 용어는 유지하지 않고 모두 제거
...
// 비규제 업종 전용 SEO 프롬프트 (후기식 - 원장님 사례 + 고객 반응 혼합)
const SEO_REVIEW_STYLE_PROMPT = `이 글은 뷰티샵 원장님이 직접 운영 경험과 고객 사례를 공유하는 후기식 블로그 글이다.
정보를 나열하는 교과서식 글이 아니라, 실제 경험이 묻어나는 생생한 글을 쓴다.

## 글쓰기 핵심 원칙
이 글은 "정보 전달"이 아니다. 원장님이 실제 경험한 사례를 통해 전문성을 자연스럽게 보여주는 글이다.
- 원장님 1인칭("저는", "제가") 자연스럽게 사용 가능
- 실제 고객 사례를 기반으로 스토리를 풀어갈 것
- 고객의 반응, 변화 과정을 자연스럽게 묘사할 것
- "~하는 것이 좋다", "~해야 한다" 같은 교과서체 절대 금지

## 후기식 글 구조 (반드시 따를 것)
1. 고객 사례로 시작: "얼마 전 이런 고민을 가진 분이 오셨거든요", "요즘 이런 문의가 정말 많아요"
2. 고객의 상태/고민 묘사: 어떤 상황이었는지 구체적으로
3. 관리 과정 공유: 어떻게 관리했는지 원장님 시점에서 자연스럽게
4. 변화/결과: 관리 후 어떻게 달라졌는지 (과장 없이 솔직하게)
5. 원장님의 팁/조언: 비슷한 고민이 있는 분들을 위한 조언

## 필수 포함 요소
- 구체적인 고객 사례 최소 1~2개 (가상이어도 현실감 있게)
- 구체적 숫자: 나이대(30대 후반), 기간(2주 뒤), 횟수(3회차) 등
- 관리 전후 변화 묘사 (과장 없이)
- 원장님만의 노하우나 관점 1가지 이상

## 고객 반응 묘사 (자연스럽게 사용)
아래 형태로 고객 반응을 자연스럽게 녹일 것:
- "처음 오셨을 때 '이게 진짜 되나요?' 하시더라고요"
- "2회차 관리 끝나고 '확실히 다르네요' 하시면서 웃으시더라고요"
- "나중에 '지인한테도 추천했어요'라고 연락을 주셨어요"
- 단, 과장된 극적 반응은 금지. 자연스럽고 현실적인 반응만 사용

## 사용 금지 (이것만 지키면 됨)
- 직접적 CTA: "예약하세요", "문의주세요", "상담받으세요", "방문해보세요" 금지
- 과대광고: "최고의", "1등", "완벽한", "효과 보장" 금지
- 정보 나열체: "~하는 것이 좋다", "~해야 한다", "~할 수 있다"를 3회 이상 반복 금지
- 교과서식 설명: "일반적으로 ~", "통상적으로 ~" 같은 딱딱한 서술 금지
- 가상 인물 이름: "김민지 씨(35세)" 같은 가상 인물 금지. 대신 "30대 후반 고객분", "직장인 고객분" 형태 사용

## 쓰면 안 되는 뻔한 패턴
- "피부관리 진행 과정은 크게 N단계로 나눌 수 있다" 같은 교과서식 구조
- "~도 중요하다", "~도 필요하다" 반복하며 항목 나열
- 원인-증상-해결법을 차례로 설명하는 백과사전식 글
- 결론에서 "꼼꼼하게 알아보고 신중하게 선택하세요" 같은 뻔한 마무리

## 대신 이렇게 써라
- "요즘 30대 고객분들 중에 이런 분이 꽤 계시거든요" → 사례로 시작
- "처음 상담할 때 피부를 보니까 이런 상태였어요" → 현장감 있는 묘사
- "그래서 이런 방향으로 관리를 잡았거든요" → 전문가 판단 공유
- "2주 뒤에 오셨을 때 확실히 달라져 있었어요" → 결과 공유
- "비슷한 고민이 있으시다면 이 부분만 먼저 체크해보세요" → 실용적 조언

## 문장 스타일
- 문장 어미: "~거든요", "~더라고요", "~잖아요", "~이에요"를 자연스럽게 섞어 사용
- 짧은 문장과 긴 문장을 섞기. 말하듯 자연스러운 호흡
- 가상 인물 스토리 금지 (이름 붙이기 금지)
- 반드시 해당 업종/키워드에 맞는 전문 지식만 사용할 것

## 의료 용어 사용 금지 (네이버 비공개 방지 - 매우 중요)
뷰티 블로그에서 의료 용어를 사용하면 네이버에서 비공개 처리될 수 있다:
- 레이저 → 광관리, 리프팅 → 탄력관리, 필러 → 볼륨관리
- 보톡스 → 주름관리, 피부과 → 피부관리실, 치료 → 관리
- 진료 → 상담, 처방 → 추천, 환자 → 고객
- 마취/마취크림 → 진정/진정크림, 의료기기 → 관리 기기

## 형식 규칙
- 반드시 한국어만 사용. 외국어 절대 금지
- 마크다운(##, **, ---, 리스트 기호) 금지, 순수 텍스트만
- 제목에 핵심 키워드 포함
- 공백 제외 최소 1,500자 이상

## 키워드 배치 (네이버 노출 핵심)
핵심 키워드를 본문에 정확히 4~6회만 포함 (7회 이상 절대 금지):
- 제목에 반드시 1회 포함
- 본문 시작 첫 2~3문장 이내 1회
- 본문 1/3 지점 1회
- 본문 2/3 지점 1회
...
## 업종 키워드 반복 제한
업종 관련 단어는 본문 전체에서 각각 10회 이하:
- 동의어 활용: 속눈썹 → 래쉬/눈가, 눈썹 → 브로우, 피부 → 결/톤/컨디션
- 시술명도 "이 관리", "해당 케어" 등으로 대체

## 광고성 글 판단 회피
- 특정 시술/서비스명 반복 강조 금지
- 홍보 문구 금지
- 정보 전달 + 실제 경험 위주로, 독자가 스스로 판단하게 유도`;

// 톤 프리셋별 프롬프트 오버라이드 (반영구 제외 업종에만 적용)
const TONE_PROMPT_OVERRIDES: Record<string, string> = {
  warm: `## 문체 스타일 오버라이드 (따뜻한 운영자 톤)
이 글은 샵을 직접 운영하는 원장님이 편하게 이야기하듯 쓴다.
딱딱한 정보 전달이 아니라, 경험에서 우러나온 진심이 느껴지는 글이다.

- 문장 어미를 자연스럽게 섞어 쓸 것: "~거든요", "~더라고요", "~인 거죠", "~잖아요", "~같아요" 를 "~합니다/~입니다"와 섞어서 사용
- 경험을 전달할 때: "샵을 운영하다 보면 이런 분들이 꽤 오시거든요", "오래 일하다 보니 느끼는 건데요", "솔직히 말씀드리면요"
- 독자에게 말을 건네듯: "혹시 이런 경험 있으신가요?", "한번 생각해보시면 좋을 것 같아요"
- 문장 길이: 짧은 문장과 긴 문장을 자연스럽게 섞기. 20자짜리도, 50자짜리도 OK
- 마무리: 확신보다는 "참고하시면 도움이 되실 거예요" 같은 따뜻한 마무리
- 절대 금지: 뉴스 기사체, 논문체, "~하는 것으로 나타났다" 같은 딱딱한 표현`,

  professional: `## 문체 스타일 오버라이드 (신뢰감 있는 전문가 톤)
이 글은 현장 경험이 깊은 전문가가 노하우를 공유하듯 쓴다.
권위적이지 않지만 확실한 근거가 느껴지는 톤이다.

- 문장 어미: "~거든요", "~더라고요"를 적절히 섞되, "~입니다", "~이에요"도 함께 사용. 전문성과 친근함의 밸런스
- 경험을 전달할 때: "오래 일하다 보니 확실히 느끼는 게 있는데요", "현장에서 수백 건 이상 보다 보면 패턴이 보이거든요"
- 근거 제시: "실제로 ~한 경우가 많아요", "통계적으로 봐도 ~인 편이에요"
- 문장 길이: 설명이 필요한 부분은 길게, 핵심은 짧게. 리듬감 있게
- 마무리: 명확한 판단 + 부드러운 톤. "이 부분은 꼭 체크하시는 게 좋아요"
- 절대 금지: "업계에서는 ~로 판단한다" 같은 기자체, 너무 차가운 관찰자 톤`,

  conversational: `## 문체 스타일 오버라이드 (친근한 대화체 톤)
이 글은 친한 언니/오빠가 경험을 바탕으로 솔직하게 알려주는 느낌으로 쓴다.
격식 없이 편안하지만, 핵심 정보는 정확하게 전달한다.

- 문장 어미: "~거든요", "~더라고요", "~잖아요", "~인데요", "~같아요"를 주로 사용. "~합니다"는 최소화
- 경험을 전달할 때: "사실 이거 되게 많이들 헷갈려하시더라고요", "솔직히 말하면요", "근데 진짜 중요한 건요"
- 독자와 대화하듯: "이거 아시는 분 별로 없는데요", "한번 들어보시면 아 그렇구나 하실 거예요"
- 문장 길이: 짧고 리듬감 있게. 말하듯 끊어서
- 중간중간 공감 표현: "맞아요, 고민되시죠", "근데 너무 걱정 안 하셔도 돼요"
- 마무리: "도움이 되셨으면 좋겠어요", "궁금한 거 있으시면 편하게 물어보세요" 같은 편안한 마무리
- 절대 금지: 교과서체, 강의체, "~해야 한다", "~하는 것이 바람직하다" 같은 딱딱한 표현`,
};
...
${selectedTitle ? `제목: ${selectedTitle}` : ''}
${treatmentInfo ? `
⚠️⚠️ 필수 반영: 우리 샵만의 특별한 시술/프로그램 정보
아래 시술/프로그램 정보는 반드시 본문에 포함해야 합니다. 빠뜨리면 글이 거부됩니다.
본문 중간~후반부에 최소 1개 문단 이상 할애하여 아래 내용을 구체적으로 다루세요.
단, "이 샵에서는", "저희 샵" 같은 직접 홍보가 아닌, 아래 형태로 언급:
- "최근에는 이런 방식의 관리법도 주목받고 있다"
- "현장에서 보면 이런 프로그램을 도입한 곳이 늘고 있다"
- "이런 기법을 적용하면 결과가 달라지는 경우가 있다"
...
- "눈썹", "문신", "반영구" 각 단어 10회 이하로만 사용 (동의어 활용: 브로우, 시술, 아트, 눈썹선, 눈썹결 등)
- 이 시술/기법만의 구체적인 원리와 메커니즘을 깊이 있게 다뤄주세요
- 여러 항목을 나열하지 말고, 하나의 관점을 깊이 파고드세요
` : `
⚠️ 업종 키워드 반복 제한:
- "속눈썹", "눈썹", "피부", "네일", "헤어" 등 업종 단어는 각각 10회 이하로만 사용
- 동의어 적극 활용: 속눈썹→래쉬/눈가/눈매, 눈썹→브로우/눈썹선, 피부→결/톤/컨디션
- 시술명도 "이 시술", "해당 관리", "이런 방식" 등으로 대체하여 반복 줄이기
`}
