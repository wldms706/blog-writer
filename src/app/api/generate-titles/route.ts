...
## 핵심 규칙: 3개의 제목은 반드시 서로 다른 스타일이어야 합니다!

### 제목 1 — 질문/공감형
독자의 고민이나 궁금증을 질문으로 던지는 스타일.
- 예: "${keyword}, 아직도 이렇게 고르고 있어요?"
- 예: "왜 같은 ${keyword}인데 결과가 다를까?"
- 예: "나만 몰랐던 ${keyword} 고르는 기준"

### 제목 2 — 정보/가이드형
전문가가 핵심 정보를 전달하는 느낌. 숫자를 활용해도 좋지만 필수는 아님.
- 예: "${keyword} 처음이라면 꼭 알아야 할 것들"
- 예: "${keyword} 전후 비교로 보는 실제 차이"
- 예: "현직 ${businessCategory} 원장이 말하는 ${keyword} 선택법"

### 제목 3 — 경험/후기형
실제 경험담이나 솔직한 이야기 느낌. 감정과 스토리가 담긴 스타일.
- 예: "${keyword} 3곳 다녀본 사람의 솔직한 비교"
- 예: "실패하고 나서야 알게 된 ${keyword} 진짜 포인트"
- 예: "${keyword} 받고 인생이 달라졌다는 말, 진짜일까?"

## 중요:
- 3개의 제목이 비슷한 구조나 어투를 가지면 안 됩니다
- 매번 숫자를 넣을 필요 없습니다. 3개 중 최대 1개만 숫자 사용
...
{
  "titles": [
    {"title": "질문/공감형 제목", "style": "질문/공감형"},
    {"title": "정보/가이드형 제목", "style": "정보/가이드형"},
    {"title": "경험/후기형 제목", "style": "경험/후기형"}
  ]
}`;
    }
    ...
    return NextResponse.json({
      titles: [
        { title: `${keyword}, 아직도 이렇게 고르고 있어요?`, style: '질문/공감형' },
        { title: `${keyword} 처음이라면 꼭 알아야 할 것들`, style: '정보/가이드형' },
        { title: `${keyword} 3곳 다녀본 사람의 솔직한 비교`, style: '경험/후기형' },
      ],
    });
