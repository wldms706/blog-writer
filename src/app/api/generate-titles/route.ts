...
- 예: "${keyword} 원장인데 ${keyword}은 추천 안 합니다"
- 예: "고객이 많은 게 좋은 ${keyword}이 아닌 이유"

**제목 2 — 숫자 + 간접증명형:**
구체적 경력/숫자로 신뢰감을 주면서 호기심 유발.
- 예: "${keyword} 12년차가 아직도 떨리는 순간"
- 예: "3000명의 고객을 만나고 깨달은 한 가지"

**제목 3 — 솔직/고백형:**
포장 없이 날것의 진심을 드러내서 공감 유도. "솔직히"로 시작하는 제목은 금지!
- 예: "사실 ${keyword} 처음 시작할 때 자신 없었습니다"
- 예: "돈보다 중요한 게 있다는 걸 늦게 알았습니다"
- 예: "${keyword} 이 일을 왜 하냐고요? 그 질문이 제일 어렵습니다"

**제목 4 — 손실 회피형:**
이 글을 안 읽으면 놓치는 게 있다는 느낌.
- 예: "${keyword} 원장 고를 때 이걸 안 보면 후회합니다"
- 예: "좋은 ${keyword} 원장과 나쁜 원장의 결정적 차이"

**제목 5 — 자아 흠집 + 공감형:**
독자의 걱정을 가볍게 건드린 뒤 안심시키는 구조.
- 예: "아직도 ${keyword}을 가격으로만 고르세요?"
- 예: "${keyword} 잘 모르겠다는 분들이 꼭 읽어야 할 글"
...
### 제목 1 — 상식 파괴형
사람들이 "${keyword}"에 대해 "당연하다"고 생각하는 상식을 정반대로 뒤집어서 "왜?"라는 궁금증을 유발하는 제목.
- 대중이 가진 기존 인식을 역발상으로 뒤집기
- "읽지 마세요", "하지 마세요" 같은 역설적 표현 활용 가능
- 예: "${keyword} 잘한다는 곳, 절대 가지 마세요"
- 예: "${keyword}은 사실 안 하는 게 맞습니다"
- 예: "${keyword} 자주 할수록 오히려 안 좋은 이유"

### 제목 2 — 손실 회피형 (금지/위험)
인간은 생존에 불리한 정보에 본능적으로 반응합니다. "이걸 모르면 손해본다"는 심리를 자극하는 제목.
- "~하면 안 된다", "~하지 마세요", "~할 확률 OO%" 등 손실/위험 암시
- 독자가 "나도 해당되는 거 아냐?" 하고 불안해서 클릭하게 만들기
- 예: "${keyword} 고를 때 이것 모르면 돈 날립니다"
- 예: "${keyword} 받기 전에 이 사진 없으면 절대 가지 마세요"
- 예: "${keyword} 실패하는 사람 90%가 놓치는 것"

### 제목 3 — 숫자 + 간접증명형
구체적 숫자와 경험/전문성의 간접증명을 결합하여 신뢰와 호기심을 동시에 주는 제목.
- 추상적 표현 대신 구체적 숫자 사용 (경력, 건수, 비교 횟수 등)
- 전문가/경험자의 간접증명으로 권위감 부여
- 예: "${keyword} 500건 해본 원장이 말하는 진짜 차이"
- 예: "8년차 ${businessCategory} 원장이 ${keyword} 고를 때 보는 딱 한 가지"
- 예: "${keyword} 5곳 비교해보고 알게 된 사실"

### 제목 4 — 자아 흠집형
독자의 자존심을 살짝 건드려서 "뭐라고?" 하는 반응을 유도. 기분 나빠짐 → 글에서 해소 → 동기부여의 3단계 구조.
- 자존심을 건드리되 인격 모독은 절대 금지
- "아직도 ~하세요?", "이것도 모르면서 ~하세요?" 같은 가벼운 도발
- 예: "아직도 ${keyword}을 가격으로 고르세요?"
- 예: "${keyword} 제대로 알지도 못하면서 비싼 곳만 찾는 이유"
- 예: "${keyword} 잘 모르는 사람이 꼭 하는 실수"
...
- 예: "${keyword} 가격이 제일 걱정이잖아요"
- 예: "${keyword} 후회할까봐 무서운 사람 여기 보세요"
- 예: "다들 ${keyword} 어디서 하냐고 물어보는데 대답이 어렵다"

## 기법 조합 원칙 (중요!):
- 위 5가지 스타일은 기본 골격이지만, 하나의 제목 안에 여러 기법을 조합하면 클릭력이 극대화됩니다
- 예: 상식 파괴 + 숫자 = "8년차 원장이 ${keyword}은 안 하는 게 낫다고 말하는 이유"
- 예: 손실 회피 + 간접증명 = "${keyword} 300건 해본 원장이 절대 추천 안 하는 유형"
  ...
  "titles": [
    ...
    if (contentType === 'branding') {
      return NextResponse.json({
        titles: [
          { title: `${keyword} 원장인데 ${keyword}은 추천 안 합니다`, style: '상식 파괴형' },
          { title: `${keyword} 10년차가 아직도 떨리는 순간`, style: '숫자+간접증명형' },
          { title: `사실 ${keyword} 처음 시작할 때 자신 없었습니다`, style: '솔직/고백형' },
          { title: `좋은 ${keyword} 원장과 나쁜 원장의 결정적 차이`, style: '손실 회피형' },
          { title: `아직도 ${keyword}을 가격으로만 고르세요?`, style: '자아 흠집+공감형' },
        ],
      });
    }
    ...
    return NextResponse.json({
      titles: [
        { title: `${keyword} 잘한다는 곳, 절대 가지 마세요`, style: '상식 파괴형' },
        { title: `${keyword} 고를 때 이것 모르면 돈 날립니다`, style: '손실 회피형' },
        { title: `${keyword} 500건 해본 원장이 말하는 진짜 차이`, style: '숫자+간접증명형' },
        { title: `아직도 ${keyword}을 가격으로 고르세요?`, style: '자아 흠집형' },
        { title: `${keyword} 가격이 제일 걱정이잖아요`, style: '솔직/공감형' },
      ],
    });
