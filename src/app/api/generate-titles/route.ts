...
## 🔴 네이버 SEO 핵심 규칙 (모든 제목에 무조건 적용)

### 규칙 1: 메인 키워드 "${safeKeyword}"는 반드시 제목 첫 5~10자 안에 배치
네이버는 제목 앞쪽 단어에 가장 큰 가중치를 줍니다.
다른 키워드(연예인, 비교 대상, 트렌드)가 먼저 나오면 그쪽으로 색인됩니다.

❌ 나쁜 예: "배수지 눈썹이 자연스러운 이유 찾다가 ${safeKeyword} 다시 보게 됐음"
   → 네이버가 "배수지 눈썹"으로 색인함. 메인 키워드 노출 실패.

✅ 좋은 예: "${safeKeyword}, 배수지처럼 자연스럽게 만드는 법"
✅ 좋은 예: "${safeKeyword} 작업할 때 배수지 레퍼런스 자주 받는 이유"

### 규칙 2: 메인 키워드의 핵심 명사(눈썹/왁싱/네일/속눈썹 등)는 제목에 단 1번만!
중복되면 키워드 스팸으로 보여서 검색 노출 ↓, 제목도 어색해짐.

❌ 나쁜 예: "${safeKeyword}, 배수지 눈썹처럼 자연스러운 눈썹 만드는 법" (눈썹 3번)
✅ 좋은 예: "${safeKeyword}, 배수지처럼 자연스럽게 만드는 법" (눈썹 1번)
✅ 좋은 예: "${safeKeyword} 작업 중 배수지 레퍼런스 받는 이유" (눈썹 1번)

연예인 이름 / 트렌드 단어 / 비교 대상은 그대로 쓰되, 핵심 명사는 반복하지 마세요.

### 규칙 3: 연예인/트렌드 단어 사용 시 → 반드시 메인 키워드 "다음에" 배치
❌ "수지 눈썹 따라하는 ${safeKeyword}"
✅ "${safeKeyword}으로 수지 따라하기"
✅ "${safeKeyword} 작업 중 수지 사진 자주 받는 이유"
