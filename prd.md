...
### 1. 글 생성 기능 ✅
- 8단계 마법사 폼 (업종 → 키워드 → 주제 → 목적 → 독자상태 → 규칙확인 → **제목선택** → 생성)
...
[현재 구현 완료]
1. 업종 선택
2. AI 키워드 추천 (지역+지수 기반) 또는 직접 입력
3. 주제 선택
4. 목적 선택
5. 독자 상태 선택
6. 규칙 확인
7. AI 제목 추천 (3개) 또는 직접 입력  ← NEW
8. 글 생성
...
│       ├── StepReader.tsx
│       ├── StepRules.tsx
│       ├── StepTitleSelect.tsx # AI 제목 추천 (NEW)
│       └── StepGenerate.tsx
├── lib/
│   ├── supabase/
│   │   ├── client.ts       # 브라우저용
...
| 날짜 | 내용 |
|------|------|
| 2026-01-28 | **AI 제목 추천 시스템 추가** - 글 생성 전 제목 후보 3개 추천 (궁금증/수치화/자극적) |
| 2026-01-28 | **블로그 지수 그룹 변경** - 최적(1~3), 준최 상위(5~7), 준최 하위(1~4), 일반(구 준최0) |
| 2026-01-28 | **스마트 키워드 추천** - 지역+업종+블로그지수 기반 AI 키워드 5개 추천 |
| 2026-01-28 | **블덱스 기반 지수 시스템** - 최적1~3, 준최0~7 체계 적용 |
| 2026-01-28 | **1인칭 표현 금지 강화** - 반영구 업종 "저는/제가/저희" 절대 금지 |
