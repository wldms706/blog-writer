...
- **카카오 소셜 로그인** ✅ (Supabase OAuth Provider)
- 미들웨어 기반 인증 보호
- 로그아웃 기능
- 세션 관리
...
### 4. 사용량 제한 ✅
- 무료 플랜: 하루 3회 (매일 자정 리셋) - 베타 기간
- 유료 플랜: 무제한 (결제 시스템 구현 후 활성화)
- 날짜 변경 시 자동 리셋

### 5. UI/UX ✅
- 반응형 레이아웃 (모바일/데스크톱)
- **모바일 최적화** ✅ (사이드바 숨김, 헤더 간소화, 버튼/입력필드 크기 조정)
- 사이드바 + 탑바 구조
- 로딩 애니메이션 + UX 메시지
- 에러 핸들링 (재시도 UI)
- **로고**: 파란 박스 + "찐" 텍스트
...
- **준최적화 5~7**: 동 + 시/구 단위 키워드 혼합 추천 ✅ (업데이트)
...
- 추천 이유 설명 포함
- 직접 입력 옵션 제공
- **동 정보 없어도 구 내 실제 동네 자동 탐색** ✅ (업데이트)
  - 예: 송파구 → 잠실동, 방이동, 문정동 등 실제 동네 사용
- **키워드 유효성 검사** ✅ (업데이트)
  - ~~공백 포함 체크~~ → 띄어쓰기 허용으로 변경
  - 한글 자모 분리 체크 (ㄱ-ㅎ, ㅏ-ㅣ) → 오타 경고
  - 특수문자 체크 → 검색어 부적합 경고
...
#### 가입 후 이름/상호명 입력 ✅
- OAuth 로그인 후 이름/상호명 입력 온보딩 페이지 표시
- 이름 필수, 상호명 선택
- 이름 미입력 시 모든 페이지에서 온보딩으로 자동 리다이렉트
- 헤더에 이메일 대신 이름 표시
- 설정 페이지에서 이름/상호명 수정 가능
- Google Sheets에 이름/상호명 동기화

### 14. 콘텐츠 안전 필터 시스템 ✅ (NEW)

#### 14-1. 의료 용어 자동 치환 ✅
- 레이저→광관리, 리프팅→탄력관리, 필러→볼륨관리 등
- 네이버 비공개 방지를 위한 2차 안전장치
...
#### 14-3. 업종 키워드 과다반복 방지 ✅
- 눈썹, 속눈썹, 입술, 피부, 네일 등 11개 업종 키워드 모니터링
- 10회 초과 시 동의어로 자동 순환 치환
- 예: 눈썹→브로우/눈썹선, 속눈썹→래쉬/눈가, 입술→립/립라인

### 15. Google Sheets 사용자 트래킹 ✅ (NEW)

#### 트래킹 데이터 ✅
- 가입일, 이메일, 이름, 상호명, 플랜, 오늘사용, 총사용, 마지막접속, 쿠폰
- 글 생성 시 자동 동기화
- 관리자 전용 수동 동기화 API (/api/tracking/sync)
...
│   │   ├── OnboardingGuard.tsx # 이름 미입력 시 리다이렉트
│   │   ├── history/        # 히스토리 페이지
...
│   └── google-sheets.ts   # Google Sheets 트래킹 동기화
...
| 날짜 | 내용 |
|------|------|
| 2026-02-14 | **업종 키워드 과다반복 방지** - 10회 초과 시 동의어 자동 치환 (11개 키워드) |
| 2026-02-14 | **외국어 자동 제거** - 러시아어/영어 후처리 필터 + 프롬프트 강화 |
| 2026-02-14 | **키워드 띄어쓰기 허용** - 공백 포함 키워드 입력 가능 |
| 2026-02-14 | **무료 제한 변경** - 하루 3회 (매일 리셋) 베타 기간 적용 |
| 2026-02-14 | **무제한 계정 추가** - mwm2020@nate.com, gkdisk9@nate.com, etang12330@gmail.com |
| 2026-02-14 | **온보딩 시스템** - 가입 후 이름/상호명 입력, 헤더 이름 표시, 설정에서 수정 |
| 2026-02-14 | **Google Sheets 트래킹 확장** - 이름/상호명 컬럼 추가 |
| 2026-01-30 | **모바일 반응형 UI** - 헤더/사이드바/설정/키워드/생성결과 모바일 최적화 |
| 2026-01-30 | **무료 사용 제한 해제** - 프로모션 기간 3회 → 999회 임시 해제 |
| 2026-01-30 | **키워드 유효성 검사** - 공백/오타/특수문자 경고 표시 |
| 2026-01-30 | **동네 자동 탐색** - 동 정보 없어도 구 내 실제 동네 자동 사용 |
| 2026-01-30 | **준최5~7 전략 변경** - 동 + 시/구 키워드 혼합 추천 |
| 2026-01-30 | **로고/파비콘** - 파란 박스 "찐" 로고, 파란 "B" 파비콘 |
